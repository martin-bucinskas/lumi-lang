...
use crate::core_parsers::comment_parser::parse_comment;
use crate::core_parsers::label_parser::parse_label_declaration;
use crate::core_parsers::opcode_parser::parse_opcode;
  ...
  fn extract_operand(token: &Token, results: &mut Vec<u8>, symbols: &SymbolTable) {
    match token {
      ...
      Token::FloatOperand { value } => {
        // using little-endian
        // todo: check this logic as this is using f64 instead of f32
        let mut wtr = vec![];
      ...
      _ => {
        error!("Invalid operand token found");
      }
    }
  }
